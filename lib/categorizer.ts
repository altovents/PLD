...
export const CATEGORIES: Record<string, CategoryInfo> = {
  ...
  transport:          { label: "Déplacements & Transport",     vat: 0.026, icon: "🚆", group: "Frais généraux" },
  ...
  matieres_premieres: { label: "Matières premières",           vat: 0.081, icon: "🏭", group: "Production" },
...
const RULES: Rule[] = [
  ...
  { pattern: /mailchimp|sendgrid|twilio/i,                      category: "saas_etranger", confidence: "high" },
  ...
  { pattern: /\bmobility\b/i,                                   category: "transport", confidence: "medium" },
  { pattern: /parking|parkhaus|park\s?&\s?ride/i,               category: "transport", confidence: "medium" },
  { pattern: /\buber\b|\bbolt\b|\bfreenow\b/i,                  category: "transport", confidence: "medium" },
  { pattern: /migrol|avia|tamoil/i,                             category: "transport", confidence: "medium" },
  ...
  { pattern: /starbucks/i,                                      category: "restaurant", confidence: "high" },
  { pattern: /dosenbach|marché|market/i,                        category: "restaurant", confidence: "medium" },
...
export function categorizeTransaction(
  ...
  for (const rule of RULES) {
    if (rule.pattern.test(text)) {
      // Always take high confidence over medium
